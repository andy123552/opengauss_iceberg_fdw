...
2. 从 iceberg_catalog.tables_internal 按 namespace/table 查表，读取 	able_uuid、metadata_location、	able_location、current_snapshot_id。
3. 根据 Catalog 中保存的 storage 配置打开 bridge storage handle。
...
7. Catalog 不解析 manifest，不扫描 data/，不做物理删除；文件级失败只透传在 JSON 中。
...
1. 校验 p_namespace、p_table 非空，older_than 大于 0，
etain_last > 0。
2. 从 	ables_internal 读取当前 metadata_location、	able_uuid、	able_location。
...
2. 从 	ables_internal 读取当前 metadata_location、	able_uuid、	able_location、current_snapshot_id。
3. 打开 bridge storage handle。

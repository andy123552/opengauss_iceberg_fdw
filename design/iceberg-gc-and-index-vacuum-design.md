...
所有 maintenance ABI 返回 `IcebergBridgeString*`，内容为 JSON。统一返回格式是为了让后续
`iceberg_catalog.gc_table(...)` 这类组合维护入口可以把多个子操作（例如
`expire_snapshots`、`remove_orphan_files`、`cleanup_old_metadata`、`vacuum_index`）的结果
稳定拼接、记录和回归校验。统一格式不意味着丢弃文件路径明细：`verbose=false` 时默认只返回
统计字段，`verbose=true` 时返回候选/删除/跳过文件明细。
...
- 组合入口同步调用各子操作，并返回各子操作 JSON 摘要。
- 组合入口不持久化 job 状态；调用方负责保存结果和重试。

...
当前 FDW 已支持 managed foreign table 的 DDL 元数据注册流程：

- `CREATE SERVER` 配置 Iceberg catalog 入口和 warehouse。
- `CREATE FOREIGN TABLE` 使用 `namespace` 和 `table_name` 创建 managed
  Iceberg 表绑定。
- FDW 会把本地外表 `relid`、Iceberg table UUID、schema 字段、unpartitioned
  spec 写入 `iceberg_catalog` 元数据表。
- 查询扫描和 DML 写入仍是接口占位，等待团队 scan/index/delta writer API 接入。

### 1. 启动 Docker openGauss
...
`iceberg_fdw` 的 managed DDL 当前依赖本地 `Catalog/` 扩展提供的
`iceberg_catalog` schema 和元数据表。该目录是外部依赖源码，不 vendor 到
FDW 扩展中。
...
当前 managed DDL 支持的列类型包括 `int2`、`int4`、`int8`、`text`、
`varchar`、`bpchar` 和固定维度 `vector(n)`。不支持通过
`metadata_location` 或 `path` 绑定已有外部 Iceberg metadata 文件。

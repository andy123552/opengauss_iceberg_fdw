...
本项目本机环境已改为 Docker 镜像运行 openGauss。之前的
`openGauss-server/` 源码仓库、binarylibs、下载包和本地编译产物已经清理，
不要再默认尝试源码编译，避免再次触发 OOM。
...
cd /home/andy/opengauss_iceberg_fdw
docker compose ps
docker stats --no-stream opengauss-iceberg-fdw
```

### 容器内连接验证

```bash
docker exec opengauss-iceberg-fdw bash -lc "su - omm -c 'gsql -d postgres -p 5432 -c \"select version();\"'"
```
...
- 当前参考提交：`75f983eb0e7bbd7725eb317aba613f42337759e5`
- 当前参考提交时间：`2026-06-01 09:59:51 +0800`
...
- 根据官方/仓库文档准备依赖。
- 编译 openGauss。
- 初始化并拉起本地 openGauss 实例。
- 记录构建命令、环境变量、失败点与规避方式。
